const precise = new ReconnectingWebSocket(`ws://${hostname}:${port}/websocket/v2/precise`);
const tosu = new ReconnectingWebSocket(`ws://${hostname}:${port}/websocket/v2`);

precise.onopen = () => {
  console.log("/v2/precise Successfully connected");
};
tosu.onopen = () => {
  console.log("/v2 Successfully connected");
};
precise.onclose = (event) => {
  console.log("/v2/precise Socket Closed Connection: ", event);
  socket.send("Client Closed!");
};
tosu.onclose = (event) => {
  console.log("/v2 Socket Closed Connection: ", event);
  socket.send("Client Closed!");
};
precise.onerror = (error) => {
  console.log("/v2/precise Socket Error: ", error);
};
tosu.onerror = (error) => {
  console.log("/v2 Socket Error: ", error);
};
...
tosu.onmessage = (event) => {
  data = JSON.parse(event.data);
};

precise.onmessage = (event) => {
    try {

/*receive*/ pdata = JSON.parse(event.data);
            convertVariable();
...
function convertVariable() {
  if (data.play.mods.name === "") {data.play.mods.name = "NM"};
  tokenValue = {
    audio: {
      fullPath: encodeURIComponent(`${data.folders.beatmap}/${data.files.audio}`)
    },
    background: {
      fullPath: encodeURIComponent(`${data.folders.beatmap}/${data.files.background}`)
    },
    modsArray: null,
    username: data.profile.name,
    banchoId: data.profile.id,
    userRankedScore: data.profile.rankedScore,
    userLevel: data.profile.level,
    userAccuracy: data.profile.accuracy,
    userPP: data.profile.pp,
    userPlayCount: data.profile.playCount,
    userGlobalRank: data.profile.globalRank,
    userCountry: data.profile.countryCode.name,
    artistRoman: data.beatmap.artist,
    artistUnicode: data.beatmap.artistUnicode,
    titleRoman: data.beatmap.title,
    titleUnicode: data.beatmap.titleUnicode,
    c100: data.play.hits["100"],
    c50: data.play.hits["50"],
    miss: data.play.hits["0"],
    sliderBreaks: data.play.hits.sliderBreaks,
    score: data.play.score,
    acc: data.play.accuracy,
    combo: data.play.combo.current,
    ingameInterfaceIsEnabled: data.settings.InterfaceVisible,
    convertedUnstableRate: data.play.unstableRate,
    dir: data.folders.beatmap,
    backgroundImageFileName: data.files.background,
    skin: data.folders.skin,
    currentBpm: data.beatmap.stats.bpm.realtime,
    grade: data.play.rank.current,
    playerHpSmooth: data.play.healthBar.smooth * 2,
    ppIfMapEndsNow: data.play.pp.current,
    K1Pressed: pdata.keys.k1.isPressed,
    K1Count: pdata.keys.k1.count,
    K2Pressed: pdata.keys.k2.isPressed,
    K2Count: pdata.keys.k2.count,
    M1Pressed: pdata.keys.m1.isPressed,
    M1Count: pdata.keys.m1.count,
    M2Pressed: pdata.keys.m2.isPressed,
    M2Count: pdata.keys.m2.count,
    hitErrors: pdata.hitErrors,
    time: pdata.currentTime / 1000,
    creator: data.beatmap.mapper,
    ...
    hp: data.beatmap.stats.hp.original,
    mBpm: data.beatmap.stats.bpm.common,
    mCS: data.beatmap.stats.cs.converted,
    mAR: data.beatmap.stats.ar.converted,
    mOD: data.beatmap.stats.od.converted,
    mHP: data.beatmap.stats.hp.converted,
    mods: data.play.mods.name.match(/.{1,2}/g).join(','),
    mp3Name: data.files.audio,
    mStars: data.beatmap.stats.stars.total,
