...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(saved => {
        ...
        if (saved.warning === true) {
            ...
            document.getElementById('enableaudiocapture').dispatchEvent(new Event('change'));
...
document.getElementById('enableaudiocapture').addEventListener('change', function(event) {
    if (this.checked) {
...
document.getElementById('enablevoid').addEventListener('change', function(event) {
    if (this.checked) {
        setTimeout(() => {
            this.checked = false;
        }, 250);
